...
current_master_name=$(pcp_node_info 10 localhost 9898 postgres 123456 $current_master_id | cut -d' ' -f1)
#Get postgres slave id
[ $current_master_id == 0 ] && current_slave_id=1 || current_slave_id=0
#Get postgres slave name
current_slave_name=$(pcp_node_info 10 localhost 9898 postgres 123456 $current_slave_id | cut -d' ' -f1)
...
AttachNodeToPgpool () {
   ...
   while [ "`pcp_attach_node 10 localhost 9898 postgres 123456  $1`" == "BackendError" ]
    do
        pcp_attach_node 10 localhost 9898 postgres 123456  $1;
...
ReattachDegeneratedSlave () {
    ...
    ssh -i $postgres_user_key root@$current_slave_name "/etc/init.d/postgresql-9.1 restart"
    #Test if postgres is running
    status=$(ssh -i $postgres_user_key postgres@$current_slave_name "if ! killall -0 postmaster; then echo 'error'; else echo 'running'; fi;")
    if [ $status == "error" ]
    then
...
SwitchOldMasterToSlave () {
    ...
    ssh -i $postgres_user_key root@$new_slave_name "/etc/init.d/postgresql-9.1 stop"
    ...
    ssh -i $postgres_user_key root@$new_master_name "/etc/init.d/postgresql-9.1 restart"
    status=$(ssh -i $postgres_user_key postgres@$new_master_name "if ! killall -0 postmaster; then echo 'error'; else echo 'running'; fi;")
    if [ $status == "error" ]
    then
    ...
    ssh -tt -i $postgres_user_key root@$new_slave_name "bash /home/postgres/data/streaming-replication.sh $new_master_name"
...
current_master_state=$(pcp_node_info 10 localhost 9898 postgres 123456 $current_master_id | cut -d' ' -f3)
current_slave_state=$(pcp_node_info 10 localhost 9898 postgres 123456 $current_slave_id | cut -d' ' -f3)
